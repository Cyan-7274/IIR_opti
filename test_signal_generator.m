% 测试信号生成+ADC抗混叠低通定点IIR滤波器功能验证
clear; clc;
load adc_cheby2_iir.mat  % 加载系数和参数
Fs = 100e6;

%% 1. 测试信号：多频正弦+阶跃+噪声
N = 2048;
t = (0:N-1)'/Fs;
f1 = 10e6; f2 = 22e6; f3 = 30e6;
...
plot(t*1e6, y_float, 'r--');
title('滤波输出：定点实现(蓝) vs 浮点理论(红虚线)');
legend('定点实现','浮点理论');
xlabel('时间(μs)'); ylabel('幅度'); grid on;

subplot(3,1,3);
plot(t*1e6, y_ref - y_float, 'k');
title('定点实现与浮点理论误差');
